...
setup() {
  if [ ! -f cloud-init.img ]; then
    # download SD card image with cloud-init; skip the test if unavailable
    if ! curl -L --fail --silent -o download.img.zip "${CLOUD_INIT_IMG_URL:-}" 2>/dev/null \
        || [ ! -s download.img.zip ]; then
      skip "cloud-init.img not available (set CLOUD_INIT_IMG_URL)"
      return
    fi
...
@test "cloud-init: flash --userdata can be downloaded" {
  run ./flash -f -d $img --userdata https://raw.githubusercontent.com/barthel/alpine-flash/master/test/resources/good.yml cloud-init.img
  assert_success
...
@test "cloud-init: flash --metadata can be downloaded" {
  run ./flash -f -d $img --userdata test/resources/good.yml --metadata https://raw.githubusercontent.com/barthel/alpine-flash/master/test/resources/meta.yml cloud-init.img
  assert_success
  assert_output_contains Downloading
  assert_output_contains Finished.
...
@test "cloud-init: flash --userdata aborts on 'not found' (404)" {
  run ./flash -f -d $img --userdata https://raw.githubusercontent.com/barthel/alpine-flash/master/test/resources/foo.bar cloud-init.img
  assert_failure
